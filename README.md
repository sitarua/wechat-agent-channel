...
- `WECHAT_OUTBOUND_MEDIA_MAX_BYTES`
- `CODEX_MODEL`
...
- 为避免大文件在微信侧出现“下载中断”，回传文件默认限制为 `50MB`；可用 `WECHAT_OUTBOUND_MEDIA_MAX_BYTES` 覆盖，例如 `52428800`
- 微信登录凭据默认保存在用户目录下
...
- To avoid large-file failures like interrupted downloads in WeChat, outbound media is capped at `50MB` by default; override with `WECHAT_OUTBOUND_MEDIA_MAX_BYTES`, for example `52428800`
